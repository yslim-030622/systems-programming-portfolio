...
운영체제 수업에서 만든 C 기반 시스템 프로젝트들을 포트폴리오용으로 정리한 저장소입니다. 과제 설명서나 로컬 테스트 산출물 대신, 직접 구현한 코드와 필요한 설명을 중심으로 정리했습니다.

### 빠른 폴더 가이드

| 폴더 | 내용 | 포트폴리오 포인트 |
| --- | --- | --- |
| `01-fuse-filesystem-wfs/` | FUSE 기반 userspace filesystem | inode, bitmap, block allocation, directory lookup, file I/O |
| `02-unix-shell-wsh/` | Unix-style shell | parser, built-ins, alias/history, fork/exec, pipelines |
| `03-xv6-stcf-scheduler/` | xv6 kernel scheduler | STCF scheduling, runtime accounting, syscall integration |
| `04-threading-concurrency-primitives/` | pthread/semaphore synchronization | writer-priority RW lock, bounded buffer |

### 프로젝트

#### WFS: Userspace Filesystem with FUSE

`01-fuse-filesystem-wfs/`
...
`04-threading-concurrency-primitives/`
...
```sh
cd 01-fuse-filesystem-wfs && make
cd ../02-unix-shell-wsh && make
cd ../04-threading-concurrency-primitives && make
```

```sh
cd 03-xv6-stcf-scheduler
...
| Folder | Project | Signal |
| --- | --- | --- |
| `01-fuse-filesystem-wfs/` | FUSE userspace filesystem | inodes, bitmaps, block allocation, directory lookup, file I/O |
| `02-unix-shell-wsh/` | Unix-style shell | parser, built-ins, alias/history, fork/exec, pipelines |
| `03-xv6-stcf-scheduler/` | xv6 kernel scheduler | STCF scheduling, runtime accounting, syscall integration |
| `04-threading-concurrency-primitives/` | pthread/semaphore synchronization | writer-priority RW lock, bounded buffer |
